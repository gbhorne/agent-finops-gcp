...
CREATE OR REPLACE TABLE `finops-gcp-agent.agent_finops_mart.run_cost_anomalies` AS
WITH baseline AS (
  SELECT
    agent_name,
    step_name,
...
recent_runs AS (
  ...
  FROM `finops-gcp-agent.agent_finops_raw.agent_cost_events` e
  JOIN baseline b
...
CREATE OR REPLACE TABLE `finops-gcp-agent.agent_finops_mart.agent_cost_daily`
...
CREATE OR REPLACE VIEW `finops-gcp-agent.agent_finops_mart.top_expensive_runs` AS
SELECT
  run_id,
  agent_name,
  workflow_name,
...
CREATE OR REPLACE VIEW `finops-gcp-agent.agent_finops_mart.budget_burn_rate` AS
WITH daily_spend AS (
  SELECT
