...
CREATE OR REPLACE VIEW `finops-gcp-agent.agent_finops_mart.runtime_daily_agent` AS
SELECT
...
CREATE OR REPLACE VIEW `finops-gcp-agent.agent_finops_mart.runtime_step_cost` AS
SELECT
  DATE(event_ts)                        AS usage_date,
  agent_name,
  workflow_name,
...
CREATE OR REPLACE VIEW `finops-gcp-agent.agent_finops_mart.runtime_mtd` AS
SELECT
  agent_name,
  environment,
...
CREATE OR REPLACE VIEW `finops-gcp-agent.agent_finops_mart.runtime_cost_per_1k` AS
SELECT
  agent_name,
  environment,
...
CREATE OR REPLACE VIEW `finops-gcp-agent.agent_finops_mart.token_trend_daily` AS
SELECT
  DATE(event_ts)            AS usage_date,
  agent_name,
  step_name,
...
CREATE OR REPLACE VIEW `finops-gcp-agent.agent_finops_mart.runtime_cost_by_urgency` AS
SELECT
  urgency,
  agent_name,
  environment,
